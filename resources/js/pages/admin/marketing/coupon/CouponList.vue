          ...
          <div class="card-body pt-0">
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5">
              ...
              <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                ...
                <th class="min-w-125px">{{ $t('coupon') }}</th>
                <th class="min-w-125px">{{ $t('reduction') }}</th>
              ...
              <tr
                ...
                <!--begin::Name=-->
                <td>
                  <router-link :to="{name: 'admin.coupons.view', params:{id: model.id}}" class="d-flex align-items-center">
                ...
                <td>
                  <label class="form-check form-switch form-check-custom form-check-solid">
                    <input
                      ...
                      :name="`models.${index}.active`"
                      type="checkbox"
                      v-model="models[index].active"
                      @input="handleToggle(index, model)">
                  </label>
                </td>
                <!--end::Active=-->
                <!--begin::Start At=-->
                <td>
                  <span :class="{
                    'text-black': $filters.isBefore(model.start_date),
                    'text-success' : $filters.isAfter(model.start_date),
                  }">
                    {{ $filters.formatDate(model.start_date) }}
                  </span>
                ...
                <td>
                  <span :class="{
                    'text-danger': $filters.isBefore(model.start_date),
                    'text-success' : $filters.isAfter(model.start_date),
                  }">
                    {{ $filters.formatDate(model.end_date) }}
                  </span>
                </td>
                <!--end::End At=-->
                ...
                <td class="text-end">
                  ...
                  <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                    ...
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.coupons.view', params: {id: model.id}}" class="menu-link px-3">{{ $t('view') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.coupons.edit', params: {id: model.id}}" class="menu-link px-3">{{ $t('edit') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
...
export default {
  ...
  setup(){
    ...
    const loadModels = () => {
      ApiService.get('/admin/coupons', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleToggle = (index, model) => {
      ApiService.put('/admin/coupons/' + model.id + '/toggle')
        .then(response => {
          if(!response.data.success){
            models.value[index].active = !models.value[index].active
          }
        })
    }
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/admin/coupons/' + model.id)
            .then(response => {
              if(response.data.success){
                loadModels()
              }
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/admin/coupons', {data: {coupons: data}})
            .then(response => {
              if(response.data.success){
