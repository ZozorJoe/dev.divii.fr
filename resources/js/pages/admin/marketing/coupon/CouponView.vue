<template>
  ...
  <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <KtToolbar>
      <!--begin::Button-->
      <router-link :to="{name: 'admin.coupons'}" class="btn btn-sm btn-active-primary me-2">{{  $t('list') }}</router-link>
      <router-link :to="{name: 'admin.coupons.edit'}" class="btn btn-sm btn-primary">{{  $t('edit') }}</router-link>
      <!--end::Button-->
    </KtToolbar>
        ...
        <div class="card mb-5 mb-xl-10">
          ...
          <div class="card-header border-0 cursor-pointer">
            ...
            <div class="card-title m-0">
              <h3 class="fw-bolder m-0">{{ $t('messages.coupons.view') }}</h3>
            </div>
            <!--end::Card title-->
          </div>
          <!--begin::Card header-->
          ...
          <div class="card-body border-top p-9">
            ...
            <div class="row mb-6">
              <!--begin::Label-->
              <label class="col-xl-2 col-lg-4 fw-bold fs-6">{{ $t('reduction') }}</label>
              <!--end::Label-->
              <!--begin::Col-->
              <div class="col-xl-10 col-lg-8 fv-row">
                <span>{{ model.reduction }}%</span>
              </div>
              <!--end::Col-->
            </div>
            <!--end::Input group-->
...
export default {
  ...
  setup(){
    ...
    const loadModel = (value) => {
      if(!value){ return; }
      const params = {params:{with:'coupons'}}
      ApiService.show('/admin/coupons/' + value, params, t('coupon'), initModel, store, t)
    }
    watch(() => id.value, () => { init() })
    onMounted(init)
