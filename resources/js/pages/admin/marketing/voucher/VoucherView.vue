<template>
  ...
  <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <KtToolbar>
      <!--begin::Button-->
      <router-link :to="{name: 'admin.vouchers'}" class="btn btn-sm btn-active-primary me-2">{{  $t('list') }}</router-link>
      <router-link :to="{name: 'admin.vouchers.edit'}" class="btn btn-sm btn-primary">{{  $t('edit') }}</router-link>
      <!--end::Button-->
    </KtToolbar>
        ...
        <div class="card mb-5 mb-xl-10">
          ...
          <div class="card-header border-0 cursor-pointer">
            ...
            <div class="card-title m-0">
              <h3 class="fw-bolder m-0">{{ $t('messages.vouchers.view') }}</h3>
            </div>
            <!--end::Card title-->
          </div>
          <!--begin::Card header-->
  ...
  setup(){
    ...
    onMounted(() => {
      store.dispatch(SET_BREADCRUMB, [
        ...
        {title: 'vouchers', route: "admin.vouchers"},
        {title: 'messages.vouchers.view'},
      ])
    })
    ...
    const loadModel = (value) => {
      if(!value){ return; }
      const params = {params:{with:'vouchers'}}
      ApiService.show('/admin/vouchers/' + value, params, t('voucher'), initModel, store, t)
    }
    watch(() => id.value, () => { init() })
    onMounted(init)
