          ...
          <div class="card-body pt-0">
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5">
              ...
              <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                ...
                <th class="min-w-125px">{{ $t('formation') }}/{{ $t('product') }}</th>
              ...
              <tr
                ...
                <!--begin::Name=-->
                <td>
                  <router-link :to="{name: 'admin.vouchers.view', params:{id: model.id}}" class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <span class="text-dark fw-bolder text-hover-primary fs-6">{{ model.code }}</span>
                    </div>
                  </router-link>
                </td>
                <!--end::Name=-->
                <!--begin::Active=-->
                <td>
                  <router-link v-if="model.formation" :to="{name: 'admin.formations.view', params:{id: model.formation.id}}" class="d-flex align-items-center">
                  ...
                  <router-link v-if="model.product" :to="{name: 'admin.products.view', params:{id: model.product.id}}" class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <span class="text-dark fw-bolder text-hover-primary fs-6">{{ model.product.name }}</span>
                    </div>
                  </router-link>
                </td>
                <!--end::Active=-->
                ...
                <td class="text-end">
                  ...
                  <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                    ...
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.vouchers.view', params: {id: model.id}}" class="menu-link px-3">{{ $t('view') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.vouchers.edit', params: {id: model.id}}" class="menu-link px-3">{{ $t('edit') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
...
export default {
  ...
  setup(){
    ...
    const loadModels = () => {
      ApiService.get('/admin/vouchers', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleToggle = (index, model) => {
      ApiService.put('/admin/vouchers/' + model.id + '/toggle')
        .then(response => {
          if(!response.data.success){
            models.value[index].is_active = !models.value[index].is_active
          }
        })
    }
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/admin/vouchers/' + model.id)
            .then(response => {
              if(response.data.success){
                loadModels()
              }
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/admin/vouchers', {data: {vouchers: data}})
            .then(response => {
              if(response.data.success){
