              ...
              <tr v-for="model in models" :key="model.id" :id="`table-row-${model.id}`">
                ...
                <td>
                  <div class="d-flex justify-content-start flex-column">
                    <span class="text-dark fw-bolder text-hover-primary fs-6">{{ $t(model.email) }}</span>
                  </div>
                </td>
                <!--end::Email=-->
                <!--begin::Action=-->
...
export default {
  ...
  setup(){
    ...
    const loadModels = () => {
      ApiService.get('/admin/newsletters', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/admin/newsletters/' + model.id)
            .then(response => {
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/admin/newsletters', {data: {newsletters: data}})
            .then(response => {
              if(response.data.success){
