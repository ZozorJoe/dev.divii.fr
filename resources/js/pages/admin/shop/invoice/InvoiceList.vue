<template>
  ...
  <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <KtToolbar>
      ...
      <a target="_blank" :href="`/admin/download/invoices/customer`" class="btn btn-sm btn-primary">Télécharger</a>
      <!--end::Button-->
    </KtToolbar>
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5">
              ...
              <tr v-for="model in models" :key="model.id" :id="`table-row-${model.id}`">
                ...
                <td>
                  <router-link
                    v-if="model.order"
                    :to="{name: 'admin.orders.view', params: {id: model.order.id}}">
                    #{{ model.order.id }}
                  </router-link>
                </td>
                <!--end::Order=-->
                ...
                <td class="text-end">
                  ...
                  <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-200px py-4" data-kt-menu="true">
                    ...
                    <div class="menu-item px-3">
                      <router-link
                        :to="{name: 'admin.invoices.view', params: {id: model.id}}"
...
export default {
  ...
  setup(){
    ...
    const loadModels = () => {
      ApiService.get('/admin/invoices', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/admin/invoices/' + model.id)
            .then(response => {
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/admin/invoices', {data: {invoices: data}})
            .then(response => {
              if(response.data.success){
