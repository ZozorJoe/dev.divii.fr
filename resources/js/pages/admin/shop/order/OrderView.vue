                  ...
                  >
                    ...
                    <td>
                      <div v-if="item.formation" class="d-flex align-items-center">
                        <!--begin::Thumbnail-->
                        <router-link :to="{name:'admin.formations.view', params:{id: item.formation.id}}" class="symbol symbol-50px">
                          <span class="symbol-label" :style="`background-image:url(/f/image-${model.id}.jpg);`"></span>
                        </router-link>
                        <!--end::Thumbnail-->
                        <!--begin::Title-->
                        <div class="ms-5">
                          <router-link :to="{name:'admin.formations.view', params:{id: item.formation.id}}" class="fw-bolder text-gray-600 text-hover-primary">{{ item.formation.name }}</router-link>
                        </div>
                        <!--end::Title-->
                      </div>
                      <div v-else-if="item.product" class="d-flex align-items-center">
                        <!--begin::Title-->
                        <div class="ms-5">
                          <router-link :to="{name:'admin.products.view', params:{id: item.product.id}}" class="fw-bolder text-gray-600 text-hover-primary">{{ item.product.name }}</router-link>
                        </div>
                        <!--end::Title-->
                      </div>
                    </td>
...
export default {
  ...
  setup(){
    ...
    const loadModel = (value) => {
      ...
      ApiService.show('/admin/orders/' + value, params, t('order'), initModel, store, t)
    }
    watch(() => id.value, () => { init() })
    onMounted(init)
