          ...
          <div class="card-body pt-0">
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5">
              ...
              <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                ...
                <th class="min-w-125px">{{ $t('credit') }}</th>
              ...
              <tr v-for="model in models" :key="model.id" :id="`table-row-${model.id}`">
                ...
                <td>
                  <router-link
                    :to="{name: 'admin.customers.view', params:{id: model.id}}"
                ...
                <!--end::Name=-->
                <!--begin::Credit=-->
                <td>{{ $tc('count.credits2', model.credit) }}</td>
                <!--end::Credit=-->
                <!--begin::Date=-->
                <td>
                ...
                <td class="text-end">
                  ...
                  <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                    ...
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.customers.view', params: {id: model.id}}" class="menu-link px-3">{{ $t('view') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.customers.edit', params: {id: model.id}}" class="menu-link px-3">{{ $t('edit') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
...
export default {
  ...
  setup(){
    ...
    const loadModels = () => {
      ApiService.get('/admin/customers', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/admin/customers/' + model.id)
            .then(response => {
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/admin/customers', {data: {customers: data}})
            .then(response => {
              if(response.data.success){
