<template>
  ...
  <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <KtToolbar>
      <!--begin::Buttons-->
      <router-link :to="{name: 'admin.disciplines'}" class="btn btn-sm btn-light btn-active-light-primary me-2">{{ $t('back') }}</router-link>
...
export default {
  ...
  setup(){
    ...
    const create = () => {
      const data = getData()
      ApiService.store('/admin/disciplines', data, t('discipline'), initModel, store, t)
    }

    const update = () => {
      const data = getData()
      ApiService.update('/admin/disciplines/' + id.value, data, t('discipline'), initModel, store, t)
    }

    const loadModel = (value) => {
      if(!value){ return; }
      ApiService.show('/admin/disciplines/' + value, {params:{with:'image,picto'}}, t('discipline'), initModel, store, t)
    }
