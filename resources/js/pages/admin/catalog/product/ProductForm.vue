<template>
  ...
  <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <KtToolbar>
      <!--begin::Buttons-->
      <router-link :to="{name: 'admin.products'}" class="btn btn-sm btn-light btn-active-light-primary me-2">{{ $t('back') }}</router-link>
...
export default {
  ...
  setup(){
    ...
    const create = () => {
      ApiService.store('/admin/products', model.value, t('speciality'), initModel, store, t)
    }

    const update = () => {
      ApiService.update('/admin/products/' + id.value, model.value, t('product'), initModel, store, t)
    }

    const loadModel = (value) => {
      if(!value){ return; }
      const params = null
      ApiService.show('/admin/products/' + value, params, t('product'), initModel, store, t)
    }
