          ...
          <div class="card-body pt-0">
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5">
              ...
              <tr v-for="model in models" :key="model.id" :id="`table-row-${model.id}`">
                ...
                <!--begin::Name=-->
                <td>
                  <router-link :to="{name: 'admin.formations.view', params:{id: model.id}}" class="d-flex align-items-center">
                ...
                <td>
                  <router-link
                    v-if="model.user"
                    :to="getViewUserRoute(model.user)"
                ...
                <td class="text-end">
                  ...
                  <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                    ...
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.formations.view', params: {id: model.id}}" class="menu-link px-3">{{ $t('view') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
                      <router-link :to="{name: 'admin.formations.edit', params: {id: model.id}}" class="menu-link px-3">{{ $t('edit') }}</router-link>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
...
export default {
  ...
  setup(){
    ...
    const params = computed(() => {
      let data = {'page' : page.value, 'with': 'user'}
      if(per_page.value){
    ...
    const loadModels = () => {
      ApiService.get('/admin/formations', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/admin/formations/' + model.id)
            .then(response => {
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/admin/formations', {data: {formations: data}})
            .then(response => {
              if(response.data.success){
    ...
    const getViewUserRoute = (user) => {
      if(user.role === "customer"){
        return {name: "admin.customers.view", params:{id: user.id}}
      }
      if(user.role === "expert"){
        return {name: "admin.experts.view", params:{id: user.id}}
      }
      if(user.role === "admin"){
        return {name: "admin.administrators.view", params:{id: user.id}}
      }
    }
