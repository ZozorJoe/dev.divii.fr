<template>
  ...
  <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <KtToolbar>
      <!--begin::Buttons-->
      <router-link :to="{name: 'admin.formations'}" class="btn btn-sm btn-light btn-active-light-primary me-2">{{ $t('back') }}</router-link>
        ...
        <div class="card mb-5 mb-xl-10">
          ...
          <form @submit.prevent="submit" id="form" method="post" class="form fv-plugins-bootstrap5 fv-plugins-framework" novalidate="novalidate">
            ...
            <div class="card-body border-top p-9">
              ...
              <div class="row mb-6">
                <!--begin::Label-->
                <label class="col-xl-2 col-lg-4 col-form-label fw-bold fs-6">{{ $t('expert') }}</label>
                <!--end::Label-->
                <!--begin::Col-->
                <div class="col-xl-10 col-lg-8 fv-row" v-if="experts.length">
                  <InputSelect name="user_id" field="name" :placeholder="$t('messages.select.expert')" class="mb-0" v-model="model.user_id" :options="experts" :rules="{}" />
                  <BaseError name="user_id"/>
                </div>
                <!--end::Col-->
              </div>
              <!--end::Input group-->
...
export default {
  ...
  setup(){
    ...
    const create = () => {
      const data = getData()
      ApiService.store('/admin/formations', data, t('formation'), initModel, store, t)
    }

    const update = () => {
      const data = getData()
      ApiService.update('/admin/formations/' + id.value, data, t('formation'), initModel, store, t)
    }
    ...
    const experts = ref([])
    const loadExperts = () => {
      ApiService.list('/admin/experts', {params:{'per-page':-1}}, t('expert'), setExperts, store, t)
    }
