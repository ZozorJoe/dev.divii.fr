          ...
          <div class="card-body pt-0">
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5">
              ...
              <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                ...
                <th class="min-w-125px">{{ $t('label') }}</th>
              ...
              <tr v-for="model in models" :key="model.id" :id="`table-row-${model.id}`">
                ...
                <!--begin::Name=-->
                <td>
                  <router-link :to="{name: 'admin.durations.view', params:{id: model.id}}" class="text-gray-800 text-hover-primary mb-1">{{ model.label }}</router-link>
                </td>
                <!--end::Name=-->
                <!--begin::Credit=-->
...
export default {
  ...
  setup(){
    ...
    const loadModels = () => {
      ApiService.get('/admin/durations', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/admin/durations/' + model.id)
            .then(response => {
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/admin/durations', {data: {durations: data}})
            .then(response => {
              if(response.data.success){
