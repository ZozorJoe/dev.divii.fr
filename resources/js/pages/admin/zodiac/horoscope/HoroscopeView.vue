<template>
  ...
  <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <KtToolbar>
      <!--begin::Button-->
      <router-link :to="{name: 'admin.horoscopes'}" class="btn btn-sm btn-active-primary me-2">{{  $t('list') }}</router-link>
      <router-link :to="{name: 'admin.horoscopes.edit'}" class="btn btn-sm btn-primary">{{  $t('edit') }}</router-link>
      <!--end::Button-->
    </KtToolbar>
        ...
        <div class="card mb-5 mb-xl-10">
          ...
          <div class="card-header border-0 cursor-pointer">
            ...
            <div class="card-title m-0">
              <h3 class="fw-bolder m-0">{{ $t('messages.horoscopes.view') }}</h3>
            </div>
            <!--end::Card title-->
          </div>
          <!--begin::Card header-->
  ...
  setup(){
    ...
    onMounted(() => {
      store.dispatch(SET_BREADCRUMB, [
        ...
        {title: 'horoscopes', route: "admin.horoscopes"},
        {title: 'messages.horoscopes.view'},
      ])
    })
    ...
    const loadModel = (value) => {
      ...
      ApiService.show('/admin/horoscopes/' + value, params, t('administrator'), initModel, store, t)
    }
    watch(() => id.value, () => { init() })
    onMounted(init)
