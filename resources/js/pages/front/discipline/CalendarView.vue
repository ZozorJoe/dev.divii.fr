<template>
  ...
  <div class="mt-10 mb-20 cc" id="cc-calendar">
    ...
    <div class="container text-white">
        ...
        <div class="d-flex flex-column flex-lg-row">
          ...
          <div class="flex-lg-row-fluid me-lg-7 ms-xl-10">
            <div class="d-flex flex-stack flex-wrap flex-grow-1 px-9 pt-4 pb-3 mx-5">
              <div class="me-2">
                <span class="fw-bolder text-white d-block fs-3 text-capitalize">{{ $filters.format(date, "MMMM YYYY") }}</span>
              </div>
      ...
      <div class="modal fade" :class="{'show': model.date, 'd-block': model.date}">
        <!--begin::Modal dialog-->
...
export default {
  ...
  setup() {
    ...
    const create = () => {
      const data = getData()
      axios.post('/api/v1/experts/' + route.params.expert  + '/disciplines/' + route.params.id + '/consultations', data)
        .then((response) => {
    ...
    const loadDurations = () => {
      ...
      ApiService.get('/experts/' + route.params.expert  + '/disciplines/' + route.params.id + '/durations', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const loadEvents = () => {
      ...
      axios.get('/api/v1/experts/' + route.params.expert  + '/disciplines/' + route.params.id + '/events', params)
        .then((response) => {
    ...
    const loadExpert = () => {
      ...
      ApiService.show('/experts/' + route.params.expert, params, t('expert'), initExpert, store, t)
    }
