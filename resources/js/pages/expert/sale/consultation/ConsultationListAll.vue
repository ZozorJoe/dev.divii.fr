        ...
        <tr v-for="model in models" :key="model.id" :id="`table-row-${model.id}`">
          ...
          <td>
            <router-link
              ...
              :to="{name: 'expert.users.view', params:{id: model.customers[0].id}}"
...
export default {
  ...
  setup(props){
    ...
    const params = computed(() => {
      ...
      data['with'] = 'customers,model.discipline'
    ...
    const loadModels = () => {
      ApiService.get('/expert/sale/consultations', {params: params.value})
        .then(({data}) => {
          if(data.success) {
    ...
    const handleDelete = (model) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete('/expert/sale/consultations/' + model.id)
            .then(response => {
    ...
    const openCancelModal = (value) => {
      ...
      Swal.fire({
        title: "Voulez-vous vraiment annuller cette consultation?",
        icon: "warning",
        showCancelButton: true,
        cancelButtonText: t('cancel'),
        confirmButtonText: t('yes'),
    ...
    const handleCancel = () => {
      ...
      ApiService.put('/expert/sale/consultations/' + data.room_id, data)
        .then(response => {
    ...
    const handleMassDelete = () => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          ApiService.delete('/expert/sale/consultations', {data: {consultations: data}})
            .then(response => {
              if(response.data.success){
